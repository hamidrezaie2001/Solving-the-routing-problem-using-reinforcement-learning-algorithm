...
def get_next_action(current_row_index1, current_column_index1,current_row_index2, current_column_index2, epsilon=0.75):
...
def convert(old_row_index1, old_column_index1, old_row_index2, old_column_index2,
                                       vert_value, matrix):
    if vert_value == 3:
        return find(vert_value, matrix)[0] ,find(vert_value, matrix)[1] ,old_row_index2, old_column_index2        
    elif vert_value == 4:
        return old_row_index1, old_column_index1,find(vert_value, matrix)[0] ,find(vert_value, matrix)[1]       
...
for episode in range(10000):
    ...
    # choose which action to take (i.e., where to move next)
        action_index = get_next_action(row_index1, column_index1,row_index2, column_index2, epsilon)
        ...
        old_q_value = q_values[old_row_index1, old_column_index1, old_row_index2, old_column_index2, action_index]
        row_index1, column_index1, row_index2, column_index2 = convert(old_row_index1, old_column_index1,old_row_index2, old_column_index2,vert_value, matrix)
        temporal_difference = reward + (discount_factor * np.max(q_values[row_index1, column_index1, row_index2, column_index2])) - old_q_value

        #update the Q-value for the previous state and action pair
        new_q_value = old_q_value + (learning_rate * temporal_difference)
        q_values[old_row_index1, old_column_index1,old_row_index2, old_column_index2, action_index] = new_q_value
